...
Band yapisi (yeni strateji - mean reversion):
  Ust Dis Tampon  = Ust Disbant + BUFFER * ATR     <- SHORT flag aç/sil
  Ust Disbant     = EMA + BAND * ATR               <- SHORT giris (crossover)
  Ust Ic Tampon   = Ust Disbant - BUFFER * ATR     <- SHORT BE TRIGGER (kar yonunde)
                                                      LONG WINRATE EXIT (kar yonunde)
  EMA (orta cizgi)                                  <- CE2 tetigi (her iki yon)

  Alt Ic Tampon   = Alt Disbant + BUFFER * ATR     <- LONG BE TRIGGER (kar yonunde)
                                                      SHORT WINRATE EXIT (kar yonunde)
  Alt Disbant     = EMA - BAND * ATR               <- LONG giris (crossover)
  Alt Dis Tampon  = Alt Disbant - BUFFER * ATR     <- LONG flag aç/sil

Ozet:
  Flag      : dis tampon (entry-tarafi)
  Giris     : dis bant (entry-tarafi crossover, geri donus)
  BE Trigger: ic tampon (entry-tarafi, 0.3 ATR kar)
  BE Exit   : dis bant (entry-tarafi, dinamik)
  Lose Exit : dis tampon (entry-tarafi, kar yonunun tersi)
  Winrate Exit: ic tampon (karsi-taraf, derin kar)
"""
