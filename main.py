...
# Bot başlangıcında bakiye okunur ve stake sabitlenir
INITIAL_BALANCE = 0.0
FIXED_STAKE = 0.0
...
def calculate_position_qty(symbol: str, price: float, stake: float) -> float:
    """Stake ve kaldıraçtan miktar hesaplar."""
    if price <= 0:
        return 0.0
...
def scan_for_entries():
    ...
    with state_lock:
        ...
        for symbol in SYMBOLS:
            try:
                # Bu coinde zaten pozisyon var mı?
                ...
                # Sinyal var, işlemi aç
                signal_dir = result["signal"]  # 'long' veya 'short'
                ...
                # Borsadan gerçek giriş fiyatını al (market emir slip yapabilir)
                time.sleep(1.5)  # Borsa pozisyonu kaydetmesi için
                ...
                # ATR değerini sakla (CE için)
                atr_value = get_atr_value(df_30m, period=14)
                if atr_value <= 0:
                    atr_value = actual_entry * 0.01  # fallback %1

                # Pozisyonu manager'a ekle
                ...
                # Bakiyeyi güncelle (sıradaki coin için)
                current_balance -= FIXED_STAKE / LEVERAGE  # margin kullanımı
...
def track_open_positions():
    """Her dakika çalışır - CE takibi ve breakeven."""
    with state_lock:
        ...
        # Bot'ta var ama borsada yoksa: SL ile kapanmış demektir
        ...
        # Açık pozisyonlar için CE takibi
        for live_p in live_positions:
            symbol = live_p["symbol"]
            pos = position_manager.get(symbol)
            if not pos:
                # Bot manager'da yok ama borsada var - manuel açılmış olabilir, dokunma
                continue
            ...
            if result["action"] == "breakeven":
                # Borsadaki SL'yi giriş fiyatına çek
                ...
                # CE tetiklendi - market ile kapat
...
def sync_positions(live_positions: list):
    """Borsa pozisyonları ile manager state'ini eşitler."""
    live_symbols = {p["symbol"] for p in live_positions}
    # Manager'da olup borsada olmayanları sil
    for sym in list(position_manager.positions.keys()):
        if sym not in live_symbols:
...
def seconds_until_next_30min_close() -> float:
    """Bir sonraki 30 dakikalık mum kapanışına kadar saniye."""
    now = datetime.now(timezone.utc)
    minute = now.minute
    next_close_minute = 30 if minute < 30 else 60
    if next_close_minute == 60:
        # Bir sonraki saatin :00'ı
...
def exit_tracker_loop():
    """1 dakikalık çıkış takip döngüsü."""
    while True:
        try:
